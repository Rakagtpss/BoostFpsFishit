--[[ 
 ULTRA EXTREME FISH IT BOOSTER
 Auto Fishing + FastReel + Mini UI + Floating Counter + Resize
...
-- CREATE UI SAFE
-- =========================
local oldGui = Player.PlayerGui:FindFirstChild("FishItExtremeUI")
if oldGui then oldGui:Destroy() end

local gui = Instance.new("ScreenGui")
gui.Name = "FishItExtremeUI"
gui.ResetOnSpawn = false
gui.Parent = Player:WaitForChild("PlayerGui")

local main = Instance.new("Frame", gui)
main.Size = UDim2.fromOffset(200,300)
...
resize.AnchorPoint = Vector2.new(0,0)
...
resize.Dragger = resize -- alias

resize.MouseMoved:Connect(function(x,y)
...
local function AutoFishing()
    ...
    pcall(function()
        ...
        if rod and rod:FindFirstChild("ReelEvent") then
            rod.ReelEvent:FireServer() -- bait
            task.wait(State.DelayBait)
            if State.FastReel then
                rod.ReelEvent:FireServer()
...
local function SetFPS()
    if setfpscap then
        if State.UnlockFPS then pcall(function() setfpscap(9999) end)
        else pcall(function() setfpscap(State.FPSLock) end)
        end
    end
end

-- =========================
-- AUTO FISHING & FAST REEL
-- =========================
local function AutoFishing()
    if not State.AutoFishing then return end
    pcall(function()
...
RunService.Heartbeat:Connect(AutoFishing)
